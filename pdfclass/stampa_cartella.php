...
$pdf->addText(25,805,8,'Cartella Clinica al '.$data_att.' - '.$intestazione);
...
$conn = db_connect();
$query="Select * From re_cartella_paziente WHERE id=".$_REQUEST['idcart'];
$rs = mssql_query($query, $conn);
$row = mssql_fetch_assoc($rs);
$paziente=$row['Cognome']." ".$row['Nome'];
$cartella=$row['codice_cartella']."/".convert_versione($row['versione']);
$regime=$row['normativa']." ".$row['regime'];
$idpaziente=$row['idutente'];
...
$pdf->ezText("Cartella Clinica $cartella\n",18,array('justification'=>'centre'));
$pdf->ezText("Utente: $paziente\n",15,array('justification'=>'centre'));
...
if($logo!=""){
	$pdf->addJpegFromFile('../images/'.$logo,265,$pdf->y-(-130),60,0);	
}
ros_logo($pdf,150,$pdf->y-10,80,150,200);
$pdf->selectFont($mainFont);
$pdf->ezNewPage();
/*
...
add_log($file,$idpaziente,'99999',$_REQUEST['idcart']);
...
foreach ($data as $line){
  ...
  $line=chop($line);
  if(strpos($line,".pdf")) {
	array_push($arr_allegati,$line);
	$pagecount = $pdf_al->setSourceFile(ALLEGATI_UTENTI_W.$line);
	$pdf->ezPageCount=$pdf->ezPageCount+$pagecount;
}
  if (strlen($line) && $line[0]=='#'){
    // comment, or new page request
    switch($line){
      case '#NP':
		$arr_tmp=array('inizio'=>$inizio,'fine'=>$fine-1,'allegati'=>$arr_allegati);
		array_push($arr_istanze,$arr_tmp);
		$arr_allegati=array();
		$inizio=$fine;	  
        $pdf->ezNewPage();
		$xx=0;
...
//  } else if (((strlen($line)>1 && $line[1]=='<') || (strlen($line) && $line[0]=='<')) && $line[strlen($line)-1]=='>') {
    ...
    $pdf->ezText(iconv('UTF-8', 'windows-1252', $line),$size,$textOptions);
	$xx++;
	//echo("fine: ".$fine." -".$line);
  }
...
//echo($xx);
if($xx>42) $fine++;
$arr_tmp=array('inizio'=>$inizio,'fine'=>$fine,'allegati'=>$arr_allegati);
array_push($arr_istanze,$arr_tmp);
...
$file_istanza=$destination_path.'tmp_pdf\cart_contenuto.pdf';
...
$file_cartella=$destination_path.'tmp_pdf\cart_contenuto.pdf';

$pdf1 =& new FPDI(); 
		 
$pagecount = $pdf1->setSourceFile($file_cartella); 
...
//Questo for prende dal file temporaneo ($file_cartella) l'ultima pagina(l'indice) e la mette all'inizio della stampa
...
//parte centrale della stampa 
 foreach ($arr_istanze as &$istanza) {	 
		 
	// initiate FPDI 
	$pagecount = $pdf1->setSourceFile($file_cartella); 
	for($i=$istanza['inizio'];$i<=$istanza['fine'];$i++){
		$tplidx = $pdf1->importPage($i, '/MediaBox');
		$pdf1->addPage(); 
		$pdf1->useTemplate($tplidx); 	
	}
	 
	foreach ($istanza['allegati'] as &$value) {
...
for($i=$fine;$i<=$pagecount-1;$i++){
	...
	$pdfcode = $pdf1->Output();
	$fp=fopen($destination_path.'tmp_pdf\stampa_cartella.pdf','wb');
	fwrite($fp,$pdf1);
	fclose($fp);
