...
{       
		...
		?>
		<tr> 
		    <td><?if($stato==2){?><img src="images/scadenza_non_rispettata.png"  title="scadenza non rispettata" /> <?}elseif($stato==1){?><img src="images/alert_scadenza.png" title="scade tra <?=$diff_in_giorni+1; if($diff_in_giorni+1==1){?> giorno<?}else{?> giorni<?}?>"/><?=$diff_in_giorni+1;}?></td> 
		    <td><?=$nome_area?></td> 
		    <td><?=$nome_modulo?></td> 
			<td>
			<?
			...
			if  ((get_permesso_modulo($idmodulo,$_SESSION['UTENTE']->get_gid())) and  (($replica==1) or (($replica==0) and (trim($ultima_compilazione)==''))))
			{
                ...
                ?>
                <a <?if(mssql_num_rows($rs_area)>0) { ?> href="#"  <? if($diff_in_giorni+1==1){?>title="scade tra <?=$diff_in_giorni+1;?> giorno "<?}elseif($diff_in_giorni>0){?> title="scade tra <?=$diff_in_giorni+1; ?> giorni"<?}else{?>title="scadenza non rispettata "<?}?>onclick="javascript:add_modulo('<?=$idarea?>','<?=$idmodulo_id?>','<?=$id_scadenze_generate_sgq?>','<?=$data_scadenza_generata?>');" ><?=$data_scadenza_generata;}?></a>
                 <? }else {
                       echo($data_scadenza_generata);
					}?>
			</td>
			...
			if  ((get_permesso_modulo_vis($idmodulo,$_SESSION['UTENTE']->get_gid())) and  (($replica==1) or (($replica==0) and (trim($ultima_compilazione)==''))))
			{
              ...
              ?>
                <a <?if(mssql_num_rows($rs_area)>0) { ?>  href="#" onclick="javascript:prev('<?=$idmodulo_id?>');" title="anteprima modulo"><img src="images/view.png" /><?}?></a>
                 <? }else {
                       echo("&nbsp;");
					}?>
			
			
			<!--<div id="anteprima_modulo" class="aggiungi "><a href="#" onclick="javascript:prev('<?=$idmodulo_id?>');" title="anteprima modulo"><img src="images/view.png" /></a></div>-->
			</td>
			...
			if  ((get_permesso_modulo($idmodulo,$_SESSION['UTENTE']->get_gid())) and  (($replica==1) or (($replica==0) and (trim($ultima_compilazione)==''))))
			{
                ...
                ?>
                <a <?if(mssql_num_rows($rs_area)>0) { ?>  href="#" title="compila modulo" onclick="javascript:add_modulo('<?=$idarea?>','<?=$idmodulo_id?>','<?=$id_scadenze_generate_sgq?>','<?=$data_scadenza_generata?>');" ><img src="images/add.png" /><?}?></a>
                 <? }else {
                       echo("&nbsp;");
					}?>
