...
function reload_review(nome_operatore,idregime,nome_modulo,data_creazione){
	...
	pagina_da_caricare="report_moduli_compilati_senza_allegati.php?op="+nome_operatore+"&regime="+idregime+"&modulo="+nome_modulo+"&data_creazione="+data_creazione;
...
?>
	...
	<?
		...
		$query="SELECT * FROM re_report_istanze_compilate_senza_allegato " . $where2 . " ORDER BY data_creazione ASC";		
		//echo $query;
		$rs = mssql_query($query, $conn);
		$totale_righe = $totale_righe + mssql_num_rows($rs);
		...
		while($row = mssql_fetch_assoc($rs)) {
			echo "<tr>
					...
					<td>".date('d/m/Y', strtotime($row['data_creazione']))."</td>";
					
					$data_scadenza = new DateTime(date('Y-m-d', strtotime($row['data_creazione'])));
