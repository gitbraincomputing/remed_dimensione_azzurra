...
function create() {
	...
	$status = $_POST['stato'];
	$reparto = empty($_POST['reparto_paziente']) ? 'NULL' : $_POST['reparto_paziente'];
	...
	$query = "INSERT INTO $tablename (note_ana,Cognome,Nome,Sesso,DataNascita,CodiceFiscale,LuogoNascita_id,stato,foto_paziente,opeins,stato_nascita,reparto) VALUES('$note_ana','$cognome','$nome','$sesso','$data_nascita','$codice_fiscale','$comune_nascita','$status','$foto_paziente1','$ope','$stato_nascita',$reparto)";	
	$result = mssql_query($query, $conn);
	...
	if($cognome_t!=""){
		$query = "INSERT INTO utenti_tutor (IdUtente,nome,cognome,indirizzo,comune_id,telefono,fax,cellulare,email,relazione_paziente,opeins,cap,comune_id_n,data_nascita,codice_fiscale,sesso,tutore_principale,tutore1,tutore2,delegato1,delegato2) VALUES('$idutente','$nome_t','$cognome_t ','$indirizzo_t','$comune_residenza_t','$telefono_t','$fax_t','$cellulare_t','$email_t','$relazione_t','$ope','$cap_t','$comune_residenza_tn','$data_nascita_t','$codice_fiscale_t','$sesso_t',$tutore_principale,$tutore1,$tutore2,$delegato1,$delegato2)";
