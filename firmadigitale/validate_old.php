<?php
	...
	require_once 'digital_doc_sign.php';
	...
	if (
		...
		(
			...
			isset($_POST['secondoallegato']) 	&& ($_POST['secondoallegato'] == 0 || $_POST['secondoallegato'] == 1) &&
	...
	)
	{
		...
		if(!$firma_cartella) {			
			...
			if (count($result_check_prec_firme) > 0) 
			{
				foreach ($result_check_prec_firme as $key=>$row_check_prec_firme) 
				{
					...
					if($num_firma_da_apporre === "" && $row_check_prec_firme['stato_firma_' . $tipoallegato] == 'a') {
						$num_firma_da_apporre = $key;
					}
				}
			}
			
			//if (count($result_check_prec_firme) > 1) 		// documento con piu firme da apporre
			if ($idmodulopadre == 134) 						// modulo "Riunioni d'equipe", l'unico ad avere il multifirma
				 $doc_multifirma = 's';
			...
			*/
			...
			else	// documento gia' firmato
			{
				...
				$query_get_allegato = "SELECT allegato from istanze_testata where id_istanza_testata = $id_istanza_testata";
		...
		$result = sign_pades_doc($access_token, $dominio_alias_certificato, $pin, $otp, $transaction_id, $pdf_content, $doc_multifirma, $num_firma_da_apporre, $rtf_landscape);
		...
		{ 
			...
			if(property_exists($result, 'signatureResult')) 
			{
				...
				if($esito == 1)	// validazione e firma eseguita
				{
					...
					if(!$firma_cartella) {
						$query_get_iit = "SELECT it.id_istanza_testata FROM istanze_testata AS it WHERE it.id_cartella = $idcartella AND it.id_modulo_versione = $idmoduloversione AND it.id_inserimento = $idinserimento $imp_query";
						...
						else $update_firma_allegati = "stato_firma_".$tipoallegato."='s', data_firma_".$tipoallegato."=getdate()";
						
						$query_set_firma = "UPDATE istanze_testata_firme SET ".$update_firma_allegati." WHERE id_istanza_testata = $id_istanza_testata_da_firmare AND id_operatore = $idoperatore";
