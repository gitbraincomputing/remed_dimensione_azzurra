...
?>
	...
	<span class="rigo_mask">
		...
		<span class="rigo_mask">
			...
			<div class="campo_mask">
				<input class="lettura" type="text" name="key_firma" value="" />
			</div>
		</span>
	</div>
...
function businessKey(id){
	...
	if(lettura=="")$(".b_key").html("<strong>businesskey non rilevata</strong> <u>rileva</u>");
	...
	function getUser(cert){
		 $.ajax({
		   ...
		   success: function(msg){
			 $("#key_firma").val($.trim(msg));
			 if($.trim(msg)=="")$(".acqbk").html("<strong>businesskey non rilevata.</strong><u>rileva nuovamente</u>");
			 else $(".acqbk").html("<strong>businesskey acquisita</strong>");
	...
	?>
		...
		<span class="rigo_mask">
			...
			<div class="campo_mask chiavebk">
				<span class="acqbk">acquisci business key da associare all'operatore</span>
				<input class="lettura" type="text" name="key_firma" id="key_firma" value="<?php echo $key; ?>" />
			</div>
		</span>
		<span class="rigo_mask">
		...
		?>
			...
			<option value="1" <?php if($usa_bk) echo "selected"; ?>>attivo</option>
			<option value="0" <?php if(!$usa_bk) echo "selected"; ?>>disattivo</option>
			</select>
		<?php			
			mssql_free_result($rs);
		?>
...
function create() {
	...
	$query = "INSERT INTO $tablename (nome,gid,descr,username,password,email,opeins,status,validazione,key_firma) VALUES('$nome','$gid','$descr','$usr','$pwd_crypt','$email','".$_SESSION['UTENTE']->get_userid()."','$status','$validazione','$key_firma')";
...
function update($id) {
	...
	if(empty($email)) error_message("Inserire l'email!");
		...
		$query = "UPDATE $tablename SET nome='$nome',gid=$gid, email='$email',key_firma='$key_firma',descr='$descr',validazione='$validazione' WHERE uid='$id'";
		$result = mssql_query($query, $conn);
		if(!$result) error_message(mssql_error());
