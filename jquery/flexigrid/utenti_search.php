...
$query="SELECT TOP 100 PERCENT * FROM (SELECT TOP $rp * FROM (SELECT TOP $z idutente,cognome, nome, datanascita,luogonascita,provnascita,codicefiscale FROM utenti $where $sort) AS FOO  $where $sort1) BAR $where $sort";
...
while ($row = mssql_fetch_assoc($result)) {
	...
	$json .= ",'".addslashes($row['datanascita'])."'";
