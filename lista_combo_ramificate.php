...
if (confirm('sei sicuro di voler cancellare la combo ramificata corrente?')){
	//document.getElementById('id_allegato').value=id;
	  $.ajax({
		   type: "POST",
		   url: "ajax_combo_moduli.php?id=2&id_combo="+id,
		   data: "",
		   success: function(msg){
			 ...
			 if(msg==""){
				 $.ajax({
					   type: "POST",
					   url: "lista_combo_ramificate_POST.php?action=del_combo&id_combo="+id,
					   data: "",
					   success: function(msg){
						 //alert( "Data Saved: " + msg );
						 reload_tipologie();
...
function reload_tipologie(){
	...
	pagina_da_caricare="lista_combo_ramificate.php";
...
<div class="logo"><img src="images/re-med-logo.png" /></div>
 ...
 <div id="briciola" style="margin-bottom:20px;">
        	<div class="elem0"><a href="#">combo ramificate</a></div>
            <div class="elem_pari"><a onclick="javascript:load_content('div','lista_combo_ramificate.php');" href="#">elenco combo ramificate</a></div>
          
        </div>

	<div class="titolo_pag">
		<div class="comandi">
			<div class="aggiungi aggiungi_left"><a onclick="javascript:load_content('div','lista_combo_ramificate.php?do=add');">aggiungi combo ramificata</a></div>
		</div>
	</div>
<?
...
if(mssql_num_rows($rs)==0){
	...
	<div class="titoloalternativo">
        <h1>elenco combo ramificate</h1>
	</div>
	<div class="info">Non esistono combo ramificate.</div>
...
{   
		...
		<tr> 
		 ...
		 <td><?=$nome?></td> 
		 <td><?=$descrizione?></td> 
		 <td><a id="<?=$id?>" onclick="javascript:load_content('div','lista_combo_ramificate.php?do=edit&id=<?=$id?>');" href="#"><img src="images/gear.png" /></a></td>
...
<div class="logo"><img src="images/re-med-logo.png" /></div>
		...
		<div id="briciola" style="margin-bottom:20px;">
           ...
           <div class="elem_pari"><a onclick="javascript:load_content('div','lista_combosgq.php');" href="#">elenco combo ramificate</a></div>
			<div class="elem_dispari"><a href="#">aggiungi combo ramificata</a></div>
          
        </div>


<form  method="post" name="form0" action="lista_combo_ramificate_POST.php" id="myForm">
...
$query="select * from campi_combo_ramificate where (idcombo='$id') and(cancella='n') and (stato=1) order by peso ASC";
//echo $query;
$rs = mssql_query($query, $conn);
