...
function countRec($tname,$conn) {
	...
	$query = "SELECT count(idnormativa) as num FROM $tname ";
...
if (!$sortname) $sortname = 'normativa';
...
$query_base="SELECT TOP $z idnormativa,normativa, descrizione from normativa $where $sort";
...
$total = countRec("normativa $where",$conn);
...
while ($row = mssql_fetch_assoc($result)) {
	if ($rc) $json .= ",";
	$json .= "\n{";
	$json .= "id:'".$row['idnormativa']."',";
	$json .= "cell:['".$row['idnormativa']."'";
	$json .= ",'".addslashes($row['normativa'])."'";
