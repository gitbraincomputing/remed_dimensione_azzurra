...
class SogeiClient {
	...
	*/
	...
	public function updateDB($idImp, $nre, $cf_medico, $codRegione, $codASLAo, 
                ...
                $franchigia, $galDirChiamAltro, $codAutenticazioneMedico, $codAutenticazioneErogatore, $codEsitoVisualizzazione, $codiceAss, $codiciCatalogo, $quantitaCatalogo, $codEsenzione, $dispReg)
        {
			...
			*/
			print_r($codiciCatalogo);
			die('count codici ->'.count($codiciCatalogo));
	...
	* */
	public function invioErogato($nre,$cfAssistito,$idImp,$dal,$al) 
	{              
		...
		$dettagli_prestazioni = $this -> get_prestazioni($idImp,$dal,$al);
		
	/*	foreach ($dettagli_prestazioni as $key => $value) {

			array_push($lista_prestazioni,
			array(
                'codProdPrest' => '', //TODO AGGIUNGERE DETTAGLI
                'descrTestoLiberoNote' => '', //TODO AGGIUNGERE DETTAGLI
				'codProdPrestErog' => $value['codice_prest_spec'] ,//Codice della prestazione specialistica effettivamente erogata, come da nomenclatore regionale
				'descrProdPrestErog' => $value['descr_presc_spec'] ,//Descrizione della prestazione specialistica effettivamente erogata, come da nomenclatore regionale
				'flagErog' => '', //TODO AGGIUNGERE DETTAGLI
                'codBranca' => $value['codice_branca_spec'] , //Codice della branca specialistica della prestazione, come da nomenclatore regionale
				'prezzo' => $value['prezzo'] ,//Tariffa della prestazione specialistica applicata al cittadino al lordo dello sconto
				'quantitaErogata' => '1' ,// Quantità effettivamente erogata. obbligatorio. Il valore indica: -per l’erogazione farmaceutica l’ effettivo numero di confezioni dispensate -per l’erogazione specialistica l’effettivo numero di prestazioni erogate (attenzione: per le prestazioni cicliche, inserite dal medico come numero di cicli da n sedute, indicare l’effettivo numero di sedute erogate)
				'dataIniErog' => $data_erogazione , //Per le ricette con prestazioni specialistiche equivale alla data di inizio erogazione.
				'dataFineErog' => $data_erogazione , // Data fine erogazione Per le ricette con prestazioni specialistiche equivale alla data di fine erogazione
				'prezzoRimborso' => $value['prezzo_rimborso'] ,// Prezzo rimborso al laboratorio Elemento obbligatorio (se assente indicare 0). Solo per prestazioni specialistiche
				'codPresidio' => '0' ,//Elemento obbligatorio (se assente indicare 0).
                'codReparto' => '0' //Elemento obbligatorio (se assente indicare 0).
			));
		}
		...
		array(
			...
			'nre' => $nre,
			'cfAssistito' => $cfAssistito_cifrato,
			'tipoOperazione'=>  OP_EROG_TOTALE,// Valore 1: erogazione totale della ricetta, tutte le prescrizioni farmaceutiche oppure tutte le prestazioni specialistiche contenute in ricetta vengono erogate allo stesso momento.
			...
			'quotaFissa' => '0' , //Ammontare della quota fissa che l’assistito deve pagare per le prestazioni specialistiche secondo  normativa nazionale e regionale
			'franchigia' => '0' , //Ammontare della franchigia che l’assistito deve pagare per le  prestazioni specialistiche
	...
	//echo json_encode($invioErogatoRichiesta);
	//$ret =  null;
