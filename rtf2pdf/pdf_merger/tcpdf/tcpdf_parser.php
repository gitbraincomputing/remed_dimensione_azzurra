...
// -----------------------------------------------------------------------------
	...
	public function __construct($data, $cfg=array()) {
		...
		$this->pdfdata = substr($data, $trimpos);	// get PDF content string
		$pdflen = strlen($this->pdfdata);			// get length
		$this->setConfig($cfg);						// set configuration parameters
		$this->xref = $this->getXrefData();			// get xref and trailer data
		$this->objects = array();					// parse all document objects
