...
<script type="text/javascript" language="javascript">
	...
	function add_allegato(<?=$_REQUEST['idcartella']?>,<?=$_REQUEST['idpaziente']?>,<?=$_REQUEST['idmoduloversione']?>,<?=$_REQUEST['idinserimento']?>) {				
...
if(isset($_SESSION['UTENTE'])) {
	...
	{
		$conn = db_connect();
		$query_note_all="SELECT note, " . $_REQUEST['campo'] . " FROM istanze_testata WHERE id_cartella = " . $_REQUEST['idcartella'] . " AND id_modulo_versione = " . $_REQUEST['idmoduloversione'] . " AND id_inserimento = " . $_REQUEST['idinserimento'];
	...
	<div class="titolo_pag">
		<h1>
		<? if($row_note_all[ $_REQUEST['campo'] ] == NULL)
			 echo "Associa allegato";
		else echo "Sostituisci l'allegato associato"; ?>
		</h1>
	</div>
	...
	<div class="blocco_centralcat">
		<div class="form">	
			<form name="form0" action="re_pazienti_sanitaria_POST.php" method="post" id="myForm" enctype="multipart/form-data">
				...
				<input type="hidden" value="<?=$_REQUEST['campo']?>" name="campo_tbl" id="campo_tbl">
				<input type="hidden" value="add_allegati_istanza" name="action"/>
		
				<div id="div_file" class="mandatory controllo_file generica" style="margin-top:20px">
					<input type="file" name="allegato" id="allegato" />
				</div>
				
				<div class="titolo_pag" style="margin-top:20px">
					<div class="comandi">		
						<div class="aggiungi aggiungi_left" style="background: transparent url(images/saveicon.png) 5px 3px no-repeat !important"><a href="#" onclick="$('#myForm').submit();">Salva allegato</a></div>
					</div>
				</div>
			</form>
		</div>
