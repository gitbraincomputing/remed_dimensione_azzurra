...
$conn = db_connect();
$conta = 0;
$where2 = " WHERE (nome_operatore <> 'admin') AND (nome_operatore <> 'Software Administrator') AND (id_paziente <> 922)";
...
function reload_review(nome_operatore,idregime,nome_modulo,data_creazione){
	...
	pagina_da_caricare="report_moduli_compilati_da_validare.php?op="+nome_operatore+"&regime="+idregime+"&modulo="+nome_modulo+"&data_creazione="+data_creazione;
...
<div class="titoloalternativo">
    <h1>Moduli compilati e firmati da validare per l'invio al FSE</h1>	         
 	<div class="stampa">
		<a onclick="javascript:(alert('Premere OK e attendere l\'elaborazione del documento..'));" 
			href="include/export_sql_xls.php?rep=rep_moduli_non_validati&where=<?=rawurlencode($where2)?>">
			<img src="images/excel-button.png">
		</a>
	</div>
</div>
...
?>
	<table id="table" class="tablesorter" cellspacing="1"> 
		...
		<tbody>
	<?
		$query="SELECT * FROM re_report_istanze_compilate_da_validare" . $where2 . " ORDER BY data_creazione ASC";		
		//echo $query;
		$rs = mssql_query($query, $conn);
		$conta = $conta + mssql_num_rows($rs);
