...
function select_pazienti($idmoduloversione){
	$conn = db_connect();
	$query="select * from re_pazienti_con_cartella order by cognome, nome";
	...
	function getCartelle(id){
		$.ajax({
		   type: "POST",
		   url: "ajax_get_cartelle.php",
		   data: "idpaziente="+id,
		   success: function(msg){			
			 $("#cartella_paz").html(trim(msg));
			}
		 });				
	}
	</script>
	...
	<div id="sanitaria">
	<div class="titolo_pag"><h1>pre-compilazione modello</h1></div>	
	<form id="myForm" method="post" name="form0" action="view_file.php?idfile=<?=$idmoduloversione?>&ngid=<?=$_SESSION['UTENTE']->get_gid()?>&sec=allsgq">
	<input id="num_cart" name="num_cart" type="hidden" value="">
		...
		<div class="riga">i campi contrassegnati con <img src="M_Validation/images/mandatory.png"> sono obbligatori</div>
		 <div class="riga"> 
			<span class="rigo_mask">
				<div class="testo_mask">Paziente da associare</div>
				<div class="campo_mask">
					<select name="paziente" class="scrittura" onchange="getCartelle(this.value);">
					<option value="0">selezionare un paziente</option>
					<?  while($row = mssql_fetch_assoc($rs)){
							$nome=pulisci_lettura($row['cognome']." ".$row['nome']);
							$idutente=$row['idutente'];	
						?>
		                <option value="<?=$idutente?>"><?=$nome?></option>
		             <?
					 }
			...
			<span class="rigo_mask">
				<div class="testo_mask">Cartella clinica</div>
				<div class="campo_mask" id="cartella_paz">
					<select name="num_cartella" class="scrittura" onchange="javascript: $('#num_cart').val(this.value);" id="num_cartella">
					<option value="0">selezionare prima un paziente</option>					
					 </select>
				</div>
			</span>			
		</div>
	...
	<div class="comandi">
		<input type="submit"  title="salva" value="apri e compila il modello" class="button_salva"/>
		<div id="aggiungi_tutor" class="elimina popupdatiClose"><a>chiudi anteprima modello</a></div>
	</div>		
	</div>
	</form>
	</div>
