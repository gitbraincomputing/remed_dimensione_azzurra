...
function del_operatore($id)
{
    ...
    $query = "DELETE FROM campi_combo WHERE idcombo = 195 AND valore='" . $id . "'";
    $result = mssql_query($query, $conn);
    exit();
}
...
function create()
{
    ...
    $ore_sett = $_POST['ore_sett'];
    $ordine_professionale = $_POST['ordine_professionale'];
    $num_iscrizione = $_POST['num_iscrizione'];
    $codice_fiscale = $_POST['codice_fiscale'];
    $alias_firma_digitale = $_POST['alias_firma_digitale'];
    ...
    $query = "INSERT INTO operatori (med_resp,titolo,bk_code,usa_firma,nome,
									gid,descr,username,password,check_pass,
									check_pass_60,data_pass_60,email,opeins,status,
									dir_sanitario,dir_tecnico,case_manager,gestore_cartella,key_firma,
									privacy_cartella,tot_ore_sett,codice_fiscale,alias_firma_digitale,ordine_professionale,
									num_iscrizione)
			VALUES('$med_resp','$titolo','$bk_code',$usabk,'$nome','$gid','$descr','$usr','$pwd_crypt','$check_pass','$check_pass_60','$datadioggi','$email','" . $_SESSION['UTENTE']->get_userid() . "','$status','$dir_san','$dir_tec','$case_manager','$gest_cart','$key_firma',$privacy_cartella,$ore_sett";

    if (trim($codice_fiscale) == "")
        $query .= ",NULL";
    else $query .= ",'$codice_fiscale'";

    if (trim($alias_firma_digitale) == "")
        $query .= ",NULL";
    else $query .= ",'$alias_firma_digitale'";

    if (trim($ordine_professionale) == "")
        $query .= ",NULL";
    else $query .= ",'$ordine_professionale'";

    if (trim($num_iscrizione) == "")
        $query .= ",NULL)";
    else $query .= ",'$num_iscrizione')";
    ...
    // cerco tutti gli operatori memorizzati nella combo "elenco operatori" atta a contenere un elenco di tutti gli OP attivi per la scelta dai moduli Diario Clinico e Riunioni d'equipe
    $query_get_max_val_combo_elenco_op = "SELECT MAX(peso) as max_peso FROM campi_combo WHERE idcombo = 195";
    $rs_get_max_val_combo_elenco_op = mssql_query($query_get_max_val_combo_elenco_op, $conn);
    $row_get_max_val_combo_elenco_op = mssql_fetch_assoc($rs_get_max_val_combo_elenco_op);

    $nuovo_peso = $row_get_max_val_combo_elenco_op['max_peso'];
    $nuovo_peso++;

    if (trim($ordine_professionale) == "")
        $valore_operatore = strtoupper(trim($nome));
    else $valore_operatore = strtoupper(trim($nome)) . " - #" . trim($num_iscrizione) . " | " . trim($ordine_professionale);

    $query_insert_elenco_op = "INSERT INTO campi_combo (idcombo, etichetta, valore, peso, stato, cancella) VALUES (195, '$valore_operatore', $id_ope, $nuovo_peso, '$status', 'n')";
    $rs_insert_elenco_op = mssql_query($query_insert_elenco_op, $conn);

    if (!empty($_POST['reparti'])) {
        foreach ($_POST['reparti'] as $rep) {
            $_sql = "INSERT INTO reparti_operatori (id_reparto, id_operatore, ope_ins)
							VALUES ($rep, $id_ope, $curr_ope)";
...
function update($id)
{
    ...
    // aggiorno il nome dell'operatore con ordine prof. e num iscrizione nella combo multivalori "elenco operatori"		
    if (trim($ordine_professionale) == "")
        $valore_operatore = strtoupper(trim($nome));
    else $valore_operatore = strtoupper(trim($nome)) . " - #" . trim($num_iscrizione) . " | " . trim($ordine_professionale);

    $query_update_combo_elenco_op = "UPDATE campi_combo SET etichetta = '$valore_operatore', stato = '$status' WHERE idcombo = 195 AND valore = '$id_ope'";
    $rs_update_combo_elenco_op = mssql_query($query_update_combo_elenco_op, $conn);

    mssql_query("DELETE FROM reparti_operatori WHERE id_operatore = $id_ope", $conn);
