...
<div  class="wrapper">
	...
	<div id="corpo-pagina">
    	<div class="sinistra_altre"><div class="strutture">
    ...
    if ($cancella_s=="y")	{
       ...
       <?=stampa_immagini_struttura($idstruttura,$ABS_PATH)?>
		...
		<?php if ((($agg_s==1)or($agg_s==2))and($tel_s!="")) {?>
        <p>Le Richieste di preventivo e di informazioni, sono gratuite e non impegnano in nessun caso l'utente che le richiede.
		</p>		
        <div class="altre_box_informazioni">
			...
			<div class="altre_box_informazioni_riga">
               ...
               <input type="text" onclick="displayCalendar(document.form1.data_in,'dd/mm/yyyy',this)" class="input_scheda2"  value="<?php if ($data_in=="") echo(date("d/m/Y"));else echo($data_in); ?>" readonly name="data_in" id="data_in" /><input type="button" style=" background-image:url(<?=WEB_ROOT?>/images/calendar.jpg); background-repeat: no-repeat; border: none; background-color: #abc6f3; width:18px; height:18px; margin: 4px 0 0 5px; float: left;" value=" " onclick="displayCalendar(document.form1.data_in,'dd/mm/yyyy',this)">
				</div>
...
Per permettere il trattamento deve acconsentire alla registrazione temporanea dei suoi dati. 
        ...
        <? }
         ...
         <p>Per una richiesta di preventivo la struttura gradisce essere contattata telefonicamente al num: <?=$tel_s?>
		</p>	
        <?php } ?>
...
<?php if ($view_map==true){?>
<script src="http://maps.google.com/maps?file=api&v=2&key=<?=$keygoogle?>" type="text/javascript"></script>
...
function showMap()
{
	...
	var lat ='<?=$latitudine_s?>';
	var longi ='<?=$longitudine_s?>';
	...
	icona_hotel.image = "http://www.hotelreservation.it/mappe/puntatore1.png";
	icona_hotel.shadow = "http://www.google.com/mapfiles/shadow50.png";
	icona_hotel.iconSize = new GSize(15, 14);
	icona_hotel.shadowSize = new GSize(22, 20);
