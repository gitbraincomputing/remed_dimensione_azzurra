...
function crea_storia(prec,succ){
	$.ajax({
	   type: "POST",
	   url: "memorizza_storia.php",
	   data: "pagina_precedente="+prec+"&pagina_chiamata="+succ,
	   success: function(data){
			//check_cancel_btn(prec);
	   }
	 });
}

function check_cancel_btn(prec){
	alert($('.comandi > .button_salva').length);
	if($('.comandi > .button_salva').length > 0) {
		$('.comandi > .button_salva').after('<input type="button" id="btn_annulla_operazione" class="btn_annulla_operazione" onclick="load_content(\'div\',\''+prec+'\');" value="Annulla Compilazione" />');
	}
}

function crea_storia_sess_var(page){
	$.ajax({
	   url: "get_var_storia.php",
	   success: function(data){
			crea_storia(data,page);
	   }
	 });
}
...
// modifica di francesco 5/10/2010 parte 1
// per eliminare il bug dovuto alla doppia pressione del tasto salva

// cerco un form nelle pagine che vengono caricate (aggiungere  disabilitaTuttiIForm();
// nella funziona inizializza() di M_forms.js ed
// abilitaTuttiIForm() in addWarning() sempre in M_forms.js)
// bind sul tasto submit,
// al submit disabilito il tasto.
// Con la parte due della mod il tasto viene riabilitato quando
// si nasconde il messaggio di stato in testa alla finestra
...
//fine mod parte1.. continua parte 2

function avvisi_loading(tipoavv) {
	
		
		// modifica di francesco 5/10/2010 parte 2
		// per eliminare il bug dovuto alla doppia pressione del tasto salva
		// disabilito tutti i pulsanti di tipo submit quando parte avviso ok
		/*
		if(tipoavv == "ok"){
			...
			//se il submit non è andato a buon fine riabilito il tasto
			$(document).find("input[type=submit]").each(function(){
				$(this).removeAttr("disabled");
			});
		}
		*/
		//continua... cerca "riabilito pulsanti mod frank 2"
		...
		$("#loading_big_"+tipoavv).fadeIn(2000,function() {			
			if(navigator.userAgent.search(/msie/i)!= -1){				
				...
				$("#layer_nero").fadeOut(2000, function() {
					...
					//riabilito pulsanti mod frank 2
...
//è stato aggiunto questo script per lanciare la funzione setAltezzaMenu anche quando viene cliccato un tasto del menu
//è saltato di mezzo secondo perchè prima si deve estendere il menù e poi va calcolata l'altazza che si è creta 
$(document).ready(function(){
