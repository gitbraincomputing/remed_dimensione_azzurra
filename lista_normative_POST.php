...
function create()
{
	$conn = db_connect();
		...
		$query = "INSERT INTO normativa (normativa,descrizione,opeins) VALUES('$nome','$descr',$opeins)";
		...
		$query = "SELECT MAX(idnormativa) FROM normativa WHERE (opeins=$opeins)";
		$result = mssql_query($query, $conn);
		if(!$result)
			{
		echo ("no");
		...
		$idnormativa=$row[0];	
		scrivi_log ($idnormativa,'normativa','ins','idnormativa');
