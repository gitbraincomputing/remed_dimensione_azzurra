...
//SE C'È .rdm ALLORA SONO NEL TEST
include_once('../include/dbengine.inc.rdm.php');
...
class SogeiClient {
   ...
   public function parseVisualizzaResponse($idImp, $ret, $giaincarica)
    {
		...
		if($codEsitoVisualizzazione == VISUAL_OK)
		{
			...
			$codProdPrestazioni = array();
			$descrProdPrestazioni = array();
			foreach($ElencoDettagliPrescrVisualErogato as $key => $value)
			{				
				if(isset($value['codProdPrest']))
					array_push($codProdPrestazioni, $value['codProdPrest']);
				if(isset($value['descrTestoLiberoNote']))
					array_push($descrProdPrestazioni, $value['descrTestoLiberoNote']);
			...
			else
			{
				...
				$this -> updateDB($idImp, $nre, $cf_medico, $codRegione, $codASLAo, 
					...
					$franchigia, $galDirChiamAltro, $codAutenticazioneMedico, $codAutenticazioneErogatore, $codEsitoVisualizzazione, $codiceAss, $codiciCatalogo, $quantitaCatalogo, $codEsenzione,$dispReg, 
					$codProdPrestazioni, $descrProdPrestazioni);
			}
		}
	...
	public function updateDB($idImp, $nre, $cf_medico, $codRegione, $codASLAo, 
				...
				$codProdPrestazioni, $descrProdPrestazioni)
        {
			...
			*/
			...
			if(count($codProdPrestazioni) == 1)
			{
				$updateRDM = "UPDATE rdm SET CodProdPrest1='$codProdPrestazioni[0]' WHERE IdImpegnativa = '$idImp' AND nre = '$nre'";
				mssql_query($updateRDM, $conn);
			}
			else if(count($codProdPrestazioni) == 2)
			{
				$updateRDM = "UPDATE rdm SET CodProdPrest1='$codProdPrestazioni[0]', CodProdPrest2='$codProdPrestazioni[1]' WHERE IdImpegnativa = '$idImp' AND nre = '$nre'";
				mssql_query($updateRDM, $conn);
			}					
			
			if(count($descrProdPrestazioni) == 1)
			{
				$updateRDM = "UPDATE rdm SET DescrLiberoNote1=\"$descrProdPrestazioni[0]\" WHERE IdImpegnativa = '$idImp' AND nre = '$nre'";
				mssql_query($updateRDM, $conn);
			}
			else if(count($descrProdPrestazioni) == 2)			
			{
				$updateRDM = "UPDATE rdm SET DescrLiberoNote1=\"$descrProdPrestazioni[0]\", DescrLiberoNote2=\"$descrProdPrestazioni[1]\" WHERE IdImpegnativa = '$idImp' AND nre = '$nre'";
				mssql_query($updateRDM, $conn);
			}	
			$data_presa_carico = date("d/m/Y H:i:s");
			$insertDataPresaInCarico = "UPDATE rdm SET DataPresaInCarico = $data_presa_carico WHERE IdImpegnativa = '$idImp'";
			mssql_query($insertDataPresaInCarico, $conn);
	...
	* */
	...
	{              
		...
		if(count($dettagli_prestazioni > 0))
		{
			...
			array_push($lista_prestazioni,
						array(
							'codProdPrest' => $value['codProdPrest1'],
							//'descrTestoLiberoNote' => $value['DescrLiberoNote1'],
							'codProdPrestErog' =>  $value['codProdPrest1'] ,//Codice della prestazione specialistica effettivamente erogata, come da nomenclatore regionale
							'descrProdPrestErog' => '', //Descrizione della prestazione specialistica effettivamente erogata, come da nomenclatore regionale
							'codCatalogoPrescr' => $value['codCatalogoErog1'],
							'codCatalogoErog' => $value['codCatalogoErog1'],
			...
			if (!empty($value['CodProdPrest2']))
			{
				...
				array_push($lista_prestazioni,
						array(
							'codProdPrest' => $value['codProdPrest2'],
							//'descrTestoLiberoNote' => $value['DescrLiberoNote1'],
							'codProdPrestErog' =>  $value['codProdPrestErog2'] ,//Codice della prestazione specialistica effettivamente erogata, come da nomenclatore regionale
							'descrProdPrestErog' => '', //Descrizione della prestazione specialistica effettivamente erogata, come da nomenclatore regionale
							'codCatalogoPrescr' => $value['codCatalogoErog2'],
							'codCatalogoErog' => $value['codCatalogoErog2'],
							'codBranca' => COD_BRANCA, //Codice della branca specialistica della prestazione, come da nomenclatore regionale
							'prezzo' => $prezzo, //Tariffa della prestazione specialistica applicata al cittadino al lordo dello sconto
			...
			array(
				...
				'nre' => $nre,
				'cfAssistito' => $cfAssistito_cifrato,
				'tipoOperazione'=>  '1',//$tipoOperazione,// Valore 1: erogazione totale della ricetta, tutte le prescrizioni farmaceutiche oppure tutte le prestazioni specialistiche contenute in ricetta vengono erogate allo stesso momento.
				...
				'tipoErogazioneSpec' => 'D', //Tipologia di erogazione di prestazioni specialistiche Valori ammessi: A="Accesso Diretto", P="Pronto Soccorso", D="Ricetta specialistica interna”
		...
		//$ret = $this -> client_invioErogato -> invioErogato($invioErogatoRichiesta);
		//print_r($ret);
		print_r($ret);
		$this->setInvioErogatoDB($ret);
		
		//$this->logger->debug('invioErogato, risultato richiesta:', array($ret));
	...
	public function callExternalServerErogato($nre,$cf,$idImp,$op,$invioErogatoRichiesta)
	{
		...
		define("URL_EXTERNAL_SOGEI_SERVER","http://localhost/rdm/remed_sogei_rdm_test.php");
...
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
	...
	public function setInvioErogatoDB($ret)
	{
		$codEsitoInserimento = $ret->codEsitoInserimento;
		
		echo($codEsitoInserimento);
		die();
		
		if($codEsitoInserimento == INSERIMENTO_OK)
		{
